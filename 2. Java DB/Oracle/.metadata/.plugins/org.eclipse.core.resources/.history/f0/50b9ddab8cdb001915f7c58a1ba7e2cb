...
import mov.com.svc.SeatService;
import mov.com.util.ConsoleUtils;


public class Seat extends JFrame implements ActionListener, Action {
	...
	// JLabel l2 = new JLabel("선택하신 영화 : " + MovieList.m.substring(0, 7)+" >> 좌석을
	// 선택해주세요.");
		...
		for (int i = 1; i < 6; i += 1) {
			for (int j = 1; j < 11; j += 1) {
				if (B.getText().equals(i + "-" + j + "공석")) {
					...
					seatsvc.book(B.getText().substring(0, 3));
					B.setText(i + "-" + j + "완료♥");
